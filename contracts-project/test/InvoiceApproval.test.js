...
describe("InvoiceApproval", function () {
  ...
  it("lets a registered Purchasing wallet create an invoice with correct totals", async function () {
    ...
    await expect(
      invoiceApproval.connect(purchasing1).createInvoice("PT Sumber Makmur", now, sampleItems, 0, "Keterangan test", 0)
    )
    ...
    expect(inv.paymentMethod).to.equal(PaymentMethod.Cash);
  });

  it("rejects invoice creation and revision without a keterangan", async function () {
    const now = Math.floor(Date.now() / 1000);
    await expect(
      invoiceApproval.connect(purchasing1).createInvoice("PT X", now, sampleItems, 0, "", 0)
    ).to.be.revertedWith("InvoiceApproval: keterangan required");

    await invoiceApproval.connect(purchasing1).createInvoice("PT X", now, sampleItems, 0, "Keterangan test", 0);
    await invoiceApproval.connect(finance1).rejectByFinance(1, "budget tidak cukup");

    await expect(
      invoiceApproval.connect(purchasing1).reviseInvoice(1, "PT X", now, sampleItems, 0, "", 0)
    ).to.be.revertedWith("InvoiceApproval: keterangan required");
  });
  ...
  it("rejects invoice creation from a non-Purchasing wallet", async function () {
    const now = Math.floor(Date.now() / 1000);
    await expect(
      invoiceApproval.connect(finance1).createInvoice("PT X", now, sampleItems, 0, "Keterangan test", 0)
    ).to.be.revertedWith("InvoiceApproval: wrong role");

    await expect(
      invoiceApproval.connect(outsider).createInvoice("PT X", now, sampleItems, 0, "Keterangan test", 0)
  ...
  it("rejects a DP amount greater than the invoice total", async function () {
    const now = Math.floor(Date.now() / 1000);
    await expect(
      invoiceApproval.connect(purchasing1).createInvoice("PT X", now, sampleItems, sampleTotal + 1, "Keterangan test", 0)
  ...
  it("lets Purchasing revise a rejected invoice and resubmit it under the same id", async function () {
    ...
    await expect(
      invoiceApproval.connect(purchasing1).reviseInvoice(1, "PT Y", now, revisedItems, 0, "Keterangan revisi", 1)
    )
    ...
    expect(inv.paymentMethod).to.equal(PaymentMethod.Transfer);
  ...
  it("rejects revision from a non-owner wallet or a non-rejected invoice", async function () {
    ...
    await expect(
      invoiceApproval.connect(outsider).reviseInvoice(1, "PT Y", now, sampleItems, 0, "Keterangan test", 0)
    ).to.be.revertedWith("InvoiceApproval: not the invoice owner");

    await expect(
      invoiceApproval.connect(purchasing1).reviseInvoice(1, "PT Y", now, sampleItems, 0, "Keterangan test", 0)
    ).to.be.revertedWith("InvoiceApproval: invoice is not rejected");
  });

  it("lists invoice ids by Purchasing wallet", async function () {
    const now = Math.floor(Date.now() / 1000);
    await invoiceApproval.connect(purchasing1).createInvoice("PT A", now, sampleItems, 0, "Keterangan test", 0);
    await invoiceApproval.connect(purchasing1).createInvoice("PT B", now, sampleItems, 0, "Keterangan test", 0);
